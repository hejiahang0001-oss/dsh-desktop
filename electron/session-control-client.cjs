...
class SessionControlClient {
  ...
  send(operation, payload) {
    if (!['inspect', 'status', 'workspace-status', 'fork', 'task-create', 'task-prompt', 'task-status', 'task-cancel'].includes(operation) || !this.child?.connected || this.pending.size >= 4) return Promise.reject(new Error('会话控制暂不可用，请稍后重试。'));
