...
const createDesktopCredentialHost = async ({ homeDir, runtime, rootDir, resourcesPath, isPackaged, crypto, provisionPlugin, terminalReadBroker }) => {
  ...
  const sessionControl = new SessionControlClient();
  return { patchPath, providerModule, toolsModule, status: () => vault.status(), sessionControl,
    redact: (text) => {
    ...
    attach: (child) => { const credentials = attachCredentialChannel(child, vault); const terminal = terminalReadBroker && attachTerminalReadChannel(child, terminalReadBroker); const control = toolsModule && sessionControl.attach(child); return () => { credentials(); terminal?.(); control?.(); }; },
