...
const createDesktopCredentialHost = async ({ homeDir, runtime, rootDir, resourcesPath, isPackaged, crypto, provisionPlugin }) => {
  ...
  return { patchPath, providerModule, status: () => vault.status(), attach: (child) => attachCredentialChannel(child, vault),
