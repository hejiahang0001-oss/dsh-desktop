...
V0.5.15 产品 Latest 继续固定官方 Harness `0.1.1-rc.2`、Electron `43.4.1` 和 pnpm `11.19.0`。固定目录现支持 `@nonamelego/dsh-catppuccin` 的原生确认安装、已审核 `0.3.0`→`0.3.1` 升级、卸载、启停和最近可用状态回退；持久事务可在启动时恢复能证明属于软件的中断变更，遇到外部编辑冲突则封锁 Profile，不覆盖用户修改。[V0.5.15 Pre-release](https://github.com/hejiahang0001-oss/dsh-desktop/releases/tag/v0.5.15) 已通过打包、覆盖安装、安装版 smoke、CI 和远端资产门禁。V0.5.4 继续保持 Stable：
