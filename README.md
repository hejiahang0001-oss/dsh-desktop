...
> V1.1.13 (`DSH-Desktop-Setup-1.1.13.exe`) is an **unpublished candidate**, adapting fixed Harness `0.1.6-alpha.1`. The official terminal is the primary new entry; the consent-based compatibility terminal remains explicitly labelled. Extra canonical session-log upload is disabled in the desktop overlay. See the [candidate notes](docs/RELEASE_NOTES_v1.1.13.md). The Stable download below remains V1.1.12 until explicitly promoted.
