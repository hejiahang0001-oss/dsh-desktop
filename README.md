...
<p align="center">
  <a href="https://github.com/hejiahang0001-oss/dsh-desktop/releases/latest/download/DSH-Desktop-Setup-0.5.0.exe"><strong>Download for Windows</strong></a>
...
1. Download [`DSH-Desktop-Setup-0.5.0.exe`](https://github.com/hejiahang0001-oss/dsh-desktop/releases/latest/download/DSH-Desktop-Setup-0.5.0.exe).
...
**V0.5.0** keeps official DeepSeek Harness `0.1.1-rc.2` and adds automatic local code checkpoints to the existing file, preview, Diff, and persistent PTY workbench:
...
The pinned runtime includes the complete official dependency closure required by the default Web profile, but it does not activate every package in the upstream monorepo or bundle community plugins. See the [plugin inventory boundary](docs/HARNESS_PLUGIN_INVENTORY.md), [validation details](docs/VALIDATION.md), and the [V0.5.0 release notes](docs/RELEASE_NOTES_v0.5.0.md).
...
- The terminal provides one persistent PowerShell PTY session with ANSI rendering and renderer-reload recovery. Terminal tabs, split panes, and checkpoint restore/rewind are not included yet. V0.5.0 creates Git checkpoints but does not expose a destructive restore action. Git accept/reject actions stay disabled while the PTY is active and are re-enabled only after the user-change protection baseline is refreshed.
...
V0.5.0 继续固定官方 Harness `0.1.1-rc.2`，保留现有中文过程、Git 审查、文件树、持久 PTY、应用预览、图片/PDF Quick Look、命令面板和可恢复布局，并增加自动代码检查点：
...
- `.env`、`.credentials*`、私钥和 secrets 等敏感路径不会写入检查点；当前版本只保存和显示状态，恢复入口在下一版单独交付；
