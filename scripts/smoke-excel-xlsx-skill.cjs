...
const main = async () => {
  ...
  if (!outputFile) {
    throw new Error('用法：node scripts/smoke-excel-xlsx-skill.cjs --output=<json> [--packaged-resources=<目录>]');
  }

  const rootDir = path.resolve(__dirname, '..');
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'dsh-excel-skill-smoke-'));
  ...
  try {
    ...
    const workspace = await synchronizeHarnessWorkspace({
      origin,
      workspacePath,
      fallbackTitle: 'V0.5.21 Excel Skill Smoke'
    });
    ...
    const excelSkill = entries.find((entry) => entry?.name === 'excel-xlsx');
    if (!excelSkill) throw new Error('Harness skill.list 未发现内置 excel-xlsx。');
    if (excelSkill.modelInvocable !== true) throw new Error('内置 excel-xlsx 没有开放给 Harness Agent 调用。');
    result = {
      ok: true,
      ...
      skill: {
        name: excelSkill.name,
        description: excelSkill.description,
        whenToUse: excelSkill.whenToUse || '',
        modelInvocable: excelSkill.modelInvocable
      },
