...
## V1.1.1 Harness alpha.2 适配（2026-08-31，候选验收完成，等待退出旧版确认）
...
- 已保留 V1.1.0 安装包回滚副本，并校验本地 32 文件的用户数据备份（不含凭据文件）。当前旧版仍在运行；未读取其聊天内容，未强制退出或执行覆盖安装，已请求用户确认关闭。`install-verified.ps1` 会在发现任何 DSH 进程时拒绝启动安装器，并在实际安装前后核对语义文件、加密 Key、Local State 和已安装资源。
- 尚待：用户确认退出旧版、覆盖安装与安装版回归、公开发布及匿名完整下载。当前电脑仍是 V1.1.0 Stable。24 小时老化、其他电脑/内网代理、真实模型 Plan 审批和自定义 SQLite 迁移均未在本次证明，不以短时检查替代。
