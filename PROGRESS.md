...
1. V0.5.5 进入权限中心：集中展示 Ask、允许、拒绝和越界确认规则，保持 Harness 为权限语义来源。
2. 页面自动聚焦与 Harness 会话选择存在启动时序差异；V0.5.4 已禁止自动聚焦建点，并保留发送前会话复核。V0.5.7 稳定化继续覆盖重载、切换会话和分支后首轮发送。
3. 官方 fork 已验证只增加会话数据且不碰代码；V0.5.5 不把“恢复代码 + 恢复会话”合并为一个模糊动作，避免扩大不可逆误操作面。
4. 当前真实仓库已有 8 个检查点 item refs，历史列表性能和键盘交互正常；接近 12 个后按 V0.5.7 计划复测，不提前引入可能过期的缓存。
