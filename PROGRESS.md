...
- 源码测试和预览完成不等于整包交付：alpha.2 的插件热卸载/全 Profile、真实模型插话、Setup/Portable、覆盖安装及数据保留尚待验收；Office kit 声明的源码仓库返回 404，Node API 对应源码可获取性仍待核实，暂不公开新包。询问官方的 Issue 尚需维护者授权。
