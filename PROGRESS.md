...
> 日期：2026-08-25
> 当前构建：V0.5.11 Latest 候选（外部扩展安全启停门禁进行中）
> 状态：V0.5.10 已发布为 Pre-release；V0.5.11 已实现受限启停、事务恢复与安装包组成基线，正在执行发布门禁；V0.5.4 继续保持 Stable
...
| 全量自动化 | 146/146 通过；最终版本文档同步后重跑 |
| 真实启停范围 | 当前 Web Profile 未声明外部插件，因此真实软件不显示任何可误触的启停按钮；固定两个扩展层仍只读 |
