...
## V1.1.11 Harness V3 / 官方文件面板（2026-09-09，开发中）
...
- 最终 591/591 测试通过；实际运行库审计、V2→V3 明文/压缩迁移、锁与备份回退、零 Agent 冷读、真实插话/停止、双文件标签和真实 Excel/Word 读取通过。修复官方自动全屏被桌面审查面板遮挡的问题，仅调整宿主边界。
- 最终安装包正在构建，覆盖安装和公开发布验收尚未完成；证据集中在 `artifacts/v1.1.11-release-candidate` 与 `docs/VALIDATION.md`。
