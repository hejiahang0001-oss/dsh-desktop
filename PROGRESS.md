...
## V1.1.3 Harness alpha.5 适配（2026-09-02，已覆盖安装，本地验收完成，未发布）

- 已确认官方最新 tag 为 `dsh-v0.1.2-alpha.5`、commit `db6bdc3576c2d4e7c965e8e3ed0c2a731eed87f5`，从当前 alpha.2 跟随 alpha.3、alpha.4、alpha.5；V1.1.0 Stable 和公开 V1.1.1 不动。
- 官方冻结 lockfile 构建、242 个 DSH 发布包校验和 39 个编译伴随 Loader 校验通过；平铺运行时为 251 包、23,866 文件、244,440,170 字节、零重解析点。许可清单为 530 项、8 类。
- 默认 JSONL-Zstd 冷兼容通过：隔离复制的 5 个会话在 alpha.2/alpha.5 下历史哈希逐项相同，5 个源文件不变，未复制凭据、未调用模型。自定义 SQLite 迁移未测试，不扩大承诺。
- 真实模型交接首次失败，证据确认 alpha.5 要求 `isSeeded` 与精确 `inheritedEventCount`，旧 `seedLength` 已废弃。先补失败测试再修复；真实往返交接随后 12/12 通过，源目录、代码、索引、草稿与历史均保留。
- 真实凭据/Office 首轮因已知的 Workspace Write PowerShell `0xC0000005` 崩溃后等待审批而超时；同时确认软件 Key 已覆盖错误环境 Key。验收改为只在隔离生成目录通过官方 `commands.execute` 显式选择 Full Access，不改变普通用户会话；Excel/Word 随机内容读取、DPAPI、源凭据不变及无明文日志全部通过。
- 源码基础矩阵 8/8 通过；真实后台任务 22/22、插话/队列 6/6、交接 12/12、凭据/Office 通过。跨工作区 XLSX/DOCX/PDF 原生拖拽、草稿连续性和真实向上按钮插话专项 3/3 通过。Office 截图增加双帧绘制等待，定向回归通过。
- 最终 NSIS、便携包与 blockmap 已生成；发布布局 `packageReady=true`，打包内核精确为 alpha.5 固定提交，零重解析点。Setup 为 188,642,919 字节、SHA-256 `2b1da8b3e00576c7260a0e4baa9d22852e009ffb93c958008051706fef60d8e7`；Portable 为 187,951,419 字节、SHA-256 `211873da5883b78ad51e241ef426357ed018de2ee0513ceaf8fe369ad19cd311`。安装包未签名，可信自动更新仍未启用。
- 打包态基础 8/8、真实模型 4/4、跨工作区文档/连续性/插话 3/3 和独立终端通过；关键 asar/额外资源与源码哈希一致。首次安装态后台测试在模型调用前遇到一次 Electron `UnknownVizError`，失败记录保留；单项重试与全新完整真实模型矩阵均通过，未修改二进制或放宽业务断言。
- 本机已由 V1.1.1 静默覆盖到 V1.1.3，安装器退出码 0，系统登记与五项核心资源哈希一致；覆盖前 33 个会话/设置文件已形成可验证备份且不含凭据文件，安装前后语义快照、加密 Key 和 Local State 哈希均不变。安装态基础 8/8、真实模型 4/4、跨工作区拖拽/连续性/插话 3/3 与终端隔离通过。
- 最终源码全量 419/419、零失败/跳过，生产依赖审计无已知漏洞，校验清单 3/3 与实际产物一致，`git diff --check` 无空白错误。首轮全量中一个测试子进程被 Node 计为文件级失败；该文件单测和随后完整复跑均通过，失败输出保留，不把单测替代全量门禁。
- 当前仍是未公开的本地 Latest 候选：未推送、未发布 GitHub。V1.1.0 Stable、公开 V1.1.1 Pre-release 与 V1.0.5 均未修改；V1.1.2 只作为被 V1.1.3 吸收的历史候选保留记录。

## V1.1.2 插话与文档拖拽修复（2026-08-31，本地包验收通过，待确认关闭/覆盖/发布）

- 用户截图同时出现插话工作区不一致、草稿不同步和文件被识别为图片拖入。源码确认：原生桌面启动目录被误用于侧栏其他工作区会话的操作校验；上游图片插件会接管所有 Files 拖入，而桌面只拦截了 drop。
- 新增当前会话上下文解析，以受信任 Harness 会话目录中的 cwd 为聊天操作目标，校验有效主会话、本机目录和操作前后的选择一致性。不改变原生终端/项目，不重启内核，不取消其他任务；移除的是错误的启动目录假设，不是会话隔离校验。
- 完整接管文件 dragenter/dragover/dragleave/drop 反馈，保留普通文字拖动、图片的官方处理链路、添加文件按钮和导入确认。取消/移出/放下均清理反馈。
- 先复现 3 项失败，再修复通过；32 项定向回归通过。全量最终 418/418、零失败/跳过，生产依赖审计通过。首轮 417/418 的版本文档门禁失败已在补齐发布说明后通过，不修改测试要求。
- 源码和最终 unpacked 均完成跨工作区专项 3/3：真实侧栏选择后的 XLSX/DOCX/PDF 完整 Chromium 原生拖入（不是只派发 drop）、草稿持久化 10 项、真实模型向上按钮/插话继续 6 项。原文件未修改、正文及文档引用进入原输入框发送链路，模型在所选会话目录写出随机标记结果。图片仍走官方处理，文字拖动不拦截。证据：`artifacts/v1.1.2-session-drag/source-acceptance`、`packaged-acceptance`。
- 最终包基础矩阵 8/8（Harness、文档、Review、Dock、连续性、IPC、Office、PDF）通过；5 项关键源码/资源逐字节一致，生产 manifest 语义一致。`packageReady=true`，内核仍为 0.1.2-alpha.2。Setup 188,873,585 字节、SHA-256 `58b8cb73a456b4a9544f12815e06baa7749acae14bfe83fe6e03253d021911af`；便携包 188,182,085 字节，清单 `dist/SHA256SUMS-v1.1.2.txt`。安装包仍未签名。
- 失败证据保留：两次测试误按重命名标题定位 blank 会话，依据上游固定显示“新会话”的实现改用工作区新会话按钮；一次模型测试命令未引用含空格的凭据路径，隔离 Key 未正确加载，修正启动脚本后完整通过；生产 package.json 会剥离开发字段，已按实际差异验证未变字段与代码字节，而非错误要求其与开发清单逐字节相同。
- 本机正式 DSH 仍运行 V1.1.1，未关闭、未覆盖；已向用户请求关闭/覆盖安装/Latest 发布确认。V1.1.2 未推送或公开发布，不把 unpacked 验收冒充安装验收。Stable V1.1.0、公开 Latest V1.1.1 和 V1.0.5 保留不变；另一台电脑的实际环境仍待用户复验。

## V1.1.1 Harness alpha.2 适配（2026-08-31，已覆盖安装并发布 Latest，公开下载校验完成）

- 用户授权开始适配。固定上游 tag `dsh-v0.1.2-alpha.2`、commit `0a53fb55bea101816fa226bb964ae2bed71c343b`；只推进 Latest，V1.1.0 Stable 原包、标签和摘要不动，V1.0.5 继续保留。
- 升级前 403/403 源码测试、生产依赖审计、冻结 lockfile 安装和 V1.1.0 unpacked 构建通过。审计目录：`artifacts/v1.1.1-harness-alpha2`。
- 已完成官方源码构建、245 个 DSH 包的发布校验、251 个编译伴随模块的 Loader 校验；最终运行时含 245 个 DSH 与 9 个 Cordis 发布包、24,354 个文件、244,911,896 字节、零重解析点。许可清单为 533 项。依赖数不等于可用插件数。
- 已适配 namespaced Remote 错误及队列竞争提示；真实后台验收还发现 `permissionPresets.current` 已从事件数组改为 Session 参数，修复创建与提交前两处权限检查。两项复现均先失败再通过；增加权限放宽时拒绝提交的回归。最终源码 408/408、零跳过，生产依赖审计通过。
- 默认 JSONL 历史：隔离复制 5 个已完成测试会话，新旧内核 API 读取共 164 条历史记录的哈希逐项一致，原始 5 个文件不变，未复制凭据、未调用模型。上游 SQLite schema 19→20，不承诺自定义 SQLite 迁移。
- 源码真实模型验收：后台任务 22/22、插话与队列 6/6、会话/工作树往返交接 12/12；模型真实读取随机 Excel/Word 文件并写出正确结果。加密凭据源不变，凭据/日志无明文 Key；无效的子进程环境变量 Key 未覆盖软件 Key。
- 已打包版本的 Harness HTTP 200、工作区/Side Chat/控制流通过；依赖闭包 493/493，插件 152 行、123 活跃、0 失败，MCP alpha.2 ready。Word/Excel/PPT 与五个 Wiki 用户 Skill 均经实际 `skill.list` 确认为模型可调用；独立打包终端验收通过。
- 初次并发测试的终端进程挂起、旧许可数量断言、隐藏启动导致的原生截图失败，以及权限参数不兼容均保留失败证据，不计入通过项。验收窗口现在先确认可见再要求真实合成截图，未以替代图片放宽检查。源码工作台 23/23（包含 80%/120%/140% 缩放）通过。
- 最终源码再次 408/408 通过，生产依赖审计通过；最终 unpacked 免费矩阵 8/8（Harness、文档拖入、Review、Dock、会话连续性、IPC 隔离、Office、PDF）与真实模型矩阵 4/4（后台 22 项、插话队列 6 项、交接 12 项、随机 Excel/Word 读取及软件 Key）均通过。实际 Office/工作台合成截图已检查。证据分别为 `source-tests-final-scoped.log`、`unpacked-release-free` 和 `unpacked-release-paid`，不等同于安装版验收。
- 最终 NSIS、便携包与 blockmap 构建成功；发布布局检查 `packageReady=true`，资源与源码修正一致。安装包 188,871,421 字节、SHA-256 `22f83313fd40ab639d4725e99eeafc79dbd774e981953fafc4003463827cb71c`，便携包 SHA-256 `ee96d375e38b9c8d2295b0a6b9e4186e09a6ad8b8908cac238acb80e6784df94`。仍未签名，自动安装关闭。
- 代码 PR #73 已合并到 `787c0683bfa8b9c5288740009d0674f2624a42a2`；PR 与主分支的三项 GitHub CI 均通过。当时 V1.1.1 仅创建 Pre-release 草稿并开始上传，后续公开发布与下载验收见下文。V1.1.0 与 V1.0.5 的 8 项远端资产 ID/大小/摘要及本地完整文件哈希逐项不变，正式 Latest 仍是 V1.1.0。
- 用户随后明确确认退出旧版、覆盖安装和发布。可见窗口正常关闭，无窗口残留进程在确认路径后结束；安装前新增并校验 33 文件备份（不含凭据文件），旧回滚安装包仍保留。NSIS 退出码 0，系统登记 `DSH Desktop 1.1.1`；安装前后 34 个语义文件逐字节一致，加密 Key 与 Local State 哈希不变，5 项关键资源与候选包一致，安装版发布布局 `packageReady=true`。证据：`installed-upgrade/registration-recheck.json`、`installed-governance.json`。最初脚本误按不带版本的 DisplayName 查找，失败记录保留；修正查询后独立回读通过，未重装或放松版本/哈希要求。
- 安装版基础交互矩阵 8/8、独立 PTY、真实模型矩阵 4/4 通过；后台 22 项、插话队列 6 项、会话交接 12 项，随机 Excel/Word 真实读取和软件 Key 优先级均验证。第一次后台验收在模型调用前遇到 `UnknownVizError`，前五项安全检查通过；保留 `installed-paid` 失败记录，未修改二进制或截图断言，使用全新配置的 `installed-paid-recheck` 完整通过。截图稳定性列为后续测试质量项。
- 正式配置重新启动并显示窗口；新 loopback 服务拒绝匿名请求（HTTP 401，符合上游认证设计），原 17 个会话存储文件与 Key 哈希不变。未读取正式聊天、未向正式会话发送测试消息，也不将此探针当成正式会话的认证页面内容验收。证据：`normal-start-recheck-result.json`；原匿名探针误期待 HTTP 200 的失败记录保留。
- 本轮重新执行源码 408/408、生产依赖审计通过。当前电脑已是 V1.1.1；GitHub 已于 2026-08-31 13:55（北京时间）发布 V1.1.1 Pre-release，发布 ID `379523580`，标签精确指向已测试代码 `787c0683bfa8b9c5288740009d0674f2624a42a2`。4 个公开附件均已匿名完整回下载，大小与 SHA-256 为 4/4 一致，下载清单的 3/3 项逐项匹配。证据：`artifacts/public-v1.1.1/verification.json` 与 `artifacts/v1.1.1-harness-alpha2/final-public-audit.json`。
- 发布后再次验证 V1.1.0 与 V1.0.5 的发布身份、目标、通道及 8 个附件 ID/大小/摘要不变，本地保留文件完整哈希亦一致；GitHub 正式 Latest 仍为 V1.1.0。当前已安装的 V1.1.1 留在正常启动状态，未向用户正式会话发送测试消息。
- 真实 GitHub 更新查询已确认：V1.1.0 能识别 V1.1.1 Pre-release，当前 V1.1.1 不误报新版本，自动下载与安装均为关闭；没有改变用户的更新偏好。24 小时老化、其他电脑/内网代理、真实模型 Plan 审批和自定义 SQLite 迁移均未在本次证明，不以短时检查替代。

## V1.1.0 Stable 晋升与旧发布清理（2026-08-31）

- 按用户明确指令，将已验收的 V1.1.0 原发布晋升为 Stable / GitHub 正式 Latest，发布 ID 379411008，目标仍为 139fab49fab6e71cd648471863cbc28341a7e009；未重新打包、替换附件或移动源码标签。本机保持 V1.1.0，无需再次覆盖安装。
- GitHub 仅保留 V1.1.0 和上一版本 V1.0.5，共 8 个公开附件；V1.0.5 仍为上一测试版本，不同时晋升 Stable。已移除 32 个旧 Release 及其 101 个附件，保留全部源码标签与 Git 历史。删除前后 69 条远端标签引用逐项一致。
- 本地 dist/backups/artifacts 内 190 个已盘点的旧安装包、便携包、blockmap 和旧校验/更新元数据已送入 Windows 回收站，总计 14,592,386,896 字节（约 13.59 GiB）。包括未发布的 V1.0.5 失败候选；保留的是与 GitHub 摘要一致的最终 V1.0.5。本地可从回收站恢复，未清空回收站，不能把移出工作目录说成已释放磁盘空间。远端旧发布已删除，不再提供下载。
- 保留版本的安装版、便携版和校验文件在 dist；V1.0.5 回滚副本仍在 backups/pre-v1.1.0-20260831。未删除源码、测试证据、运行时依赖或用户会话/配置/Key；不相关 CAS30 文件保持原样。
- 本次重新核验 8 个附件的本地 SHA-256、远端资产 ID/大小/摘要不变，匿名 HEAD 全部 200，匿名重新下载两份校验清单并核对；此前完整二进制匿名下载验收记录仍保留。正式 Latest 安装链接可用，真实 GitHub 更新检查从 V1.0.5 能识别 V1.1.0，当前 V1.1.0 不误报更高版本。发布/更新定向测试 8/8 通过。
- README 中英文下载、发布说明和计划已同步新 Stable。发布信任边界不因晋升改变：未签名、可信自动安装未启用、24 小时/多日老化尚未完成。原包仍存在旧 Stable 提示文案和 EXE 运行时版本显示问题，已列入下一小版，不静默替换本版二进制。未来日常迭代继续走 Pre-release，Stable 仍仅由用户单独下令更新。
- 本地审计目录：artifacts/stable-1.1.0-cleanup-20260831，包含清理前清单、回收记录及晋升后公开访问与摘要复核。下文旧版本记录反映历史验收状态，不表示对应安装包仍保留。

## V1.1.0（已覆盖、已发布并匿名校验）

- 2026-08-31 已完成从 V1.0.2 到 V1.1.0 的五次版本交付。本机已覆盖 V1.1.0；403/403 源码门禁、最终包和已安装 UI/真实模型验收通过。首次 GitHub Pre-release v1.1.0 目标 139fab49fab6e71cd648471863cbc28341a7e009，四个附件匿名回下载大小与 SHA-256 为 4/4 一致（artifacts/public-v1.1.0/verification.json）；本地清单 3/3 核对通过。首次发布时 Stable 为 V0.5.4；之后已按用户明确指令晋升 V1.1.0，见上方记录。
- 最终 Setup：188,767,301 字节，SHA-256 465208D87D3EA7C811C7E909AEC1ADA18850BB43A5ECE6BE0EE3B152B03CA6BC；Portable：188,075,787 字节，SHA-256 1F89D1F3C5BB981612C1019058B1FAAC6B66157F45F5DD57A5B36A5E5ADB470D。回滚包为 backups/pre-v1.1.0-20260831 中已验证的 V1.0.5；失败候选曾在 superseded-before-index-fix 中另存，未对外发布，现已按上述清理指令回收。

以下保留开发与验收过程记录，包含当时的阶段状态；最终交付以上述汇总为准。

- 覆盖安装退出 0，Windows 安装记录为 DSH Desktop 1.1.0；安装 app.asar 与最终包一致。覆盖前后原有语义快照均为 E87346B8785527E43B339846DF16F3AF6105027F532858E3BC7F96A7A6E31EFD。七组安装 UI/隔离回归、真实后台 22/22、交接 12/12、插话/队列 6/6 和终端检查全部通过（artifacts/v1.1-installed-final）。正常配置启动已连接本机 Harness，未提交实际用户任务。
- 后续质量待办与已完成事实分开：真实项目 24 小时/多日老化、签名证书与可信自动更新仍未完成；Windows EXE 文件属性仍继承 Electron 43.4.1 元数据（应用内与安装记录均为 1.1.0），后续小版单独校正，不把运行时版本当产品版本。
- 发布说明明确工作树复制边界：软件托管 Key 和源目录未提交资料不复制；Git 已提交文件仍按提交检出，包括已提交依赖或误提交的敏感文件，不能把目录隔离当内容脱敏。当前原生提示中的“Key/依赖不复制”仅适用于软件 Key 与未提交的本机依赖，下一小版应进一步消除这一文案歧义。

- 最终包治理 packageReady=true，内核固定插件、Office/Wiki 及 pnpm/终端依赖齐全；仍未签名，自动安装保持禁用。Setup 188,767,301 字节，app.asar 为 8F5492C5DFBC71F9F3C62CEFD83753945071FB834C1A5150236F273E0B6372BD。最终解包版真实后台 22/22 通过（artifacts/v1.1-unpacked-background-final/result.json）。PR #69 与 main CI 33337103543 全部通过；正在覆盖安装，GitHub 资产只上传到未公开草稿。

- 最终整合源码 403/403 通过（artifacts/v1.1-index-final-tests.log）。真实模型 22/22 检查通过，包括三个后台回合、一次真实计时触发、并行前台回复、实际内核重启与不重复提交、历史按钮滚动可达，以及点击历史打开准确会话（artifacts/v1.1-index-background-paid/result.json）。耗时 51 秒的连续验收不等于 24 小时老化测试。

- 已实现独立工作树任务、手动/一次性/每天/固定间隔计划、每日运行上限、单任务防重叠、运行身份先持久化再提交和状态核对。
- 原生任务面板包括任务示例、准确本机时间确认、运行历史、打开会话、停止/暂停、异常核对、归档和释放名额；保留分支、会话及目录。
- 每次运行建立新的 Harness 会话并明确设为 Workspace Write + Ask；不自动批准。托盘保活与明确退出确认已接入。
- 真实内核验证修正了 Agent 局部服务读取方式和公开 prompt API 必需的 AbortSignal；失败测试记录已保留，未发生自动重发。仍在执行真实连续运行和发布门禁，不能据此宣称已发布 V1.1。
- 真实模型已连续验证两次新会话文件产出、一次真实计时触发、前台同时回复、内核进程重启后的准确回执核对和不重复提交；21 项检查通过。已修复任务列表刷新返回结构和固定页脚遮挡键盘滚动目标的问题。另补充晚到状态回调不能复活已归档记录的保护；安装与公开发布尚未完成。

## V1.0.5（已覆盖、已发布并匿名校验）

- 已公开为 GitHub Pre-release v1.0.5，四个附件均为 uploaded；匿名回下载 4/4 大小与 SHA-256 一致，清单 3/3 与本地相符，证据 artifacts/public-v1.0.5/verification.json。Stable V0.5.4 未变化。最终 Setup 摘要为 2E59C2B1F4784E015DC120127A91CDD97CB863FC43B880CBFCB6F36D40F98A4F，旧失败候选未发布。

以下为该版早期验收与修正过程，最终状态以上述交付结果为准。

- 最终修正包已覆盖，安装退出 0；安装 asar 为 5D9FB15C05268D777DB12B983C4EFDBB85968D987A3BC7844F1CC61B5CF0E3D0。安装前后语义快照均为 E87346B8785527E43B339846DF16F3AF6105027F532858E3BC7F96A7A6E31EFD。七组安装 UI/隔离回归、真实模型交接 12/12、队列/插话 6/6 和固定终端检查通过（artifacts/v1.0.5-installed-index-final）。PR #70 与 main CI 33336876792 通过，最终标签目标 8cc309245dec5e6a5babcf5e81fc34ac14bea747；公开资产仍在上传草稿。

- 最终安装验收发现 Git 状态刷新/检查点只读校验争用 index.lock，原目录已自动回滚。修正为私有暂存区副本生成 tree，并禁用状态刷新的可选写锁；真实写操作仍遵守外部锁。新增锁占用、split-index、损坏 index 不降级 HEAD 回归，389/389 源码测试通过，真实付费往返 12/12 通过（artifacts/v1.0.5-index-paid/result.json）。正在重新打包，不发布旧草稿资产。

- 已实现按会话分隔的本机文字草稿、文档引用与附件记录，重载恢复、清空、过期写入和伪造令牌保护；不宣称图片/富文本装饰完整备份。
- 已实现原生确认的本机→工作树→本机交接，保留原对话、未暂存/已暂存/未跟踪代码和恢复点；原目录被其他人修改时拒绝覆盖。
- 真实付费模型往返测试通过：目标保留原对话随机标记并在目标目录写出正确文件；切换不重启 Harness。失败尝试保留，按问题修正 Agent 创建与重载选中流程后重新验收。
- 387 项源码门禁通过；真实模型已验证排队→停止保留→继续原队列→写出随机标记，以及插话得到回复并正确清理原草稿。修正了已停止会话不能走上游 steer 队列入口、从按钮回焦后仅删除一个字，以及 DOM 停止状态滞后阻止交接的问题；原失败证据保留。最终包已再次覆盖，安装 asar 为 CFD9D132F16105FDB607740C7E5450F07AE1B09409D12C874E06E68D279D1EC1，原语义数据摘要保持 E87346B8785527E43B339846DF16F3AF6105027F532858E3BC7F96A7A6E31EFD。公开版本仍待最终安装验收完成后发布。

## V1.0.4（已覆盖、已发布并匿名校验）

- 原生隔离工作台六个工具停靠、弹出和重新停靠已实现；真实 Harness/PTY 界面检查通过，紧凑窗口与缩放下输入区域仍可用。
- 只读终端工具已经注册，每次原生确认、当前前台会话和工作区校验、最多 8,000 字符、已知凭据遮蔽；不会给予网络页面终端写入权限。
- 368/368 源码测试通过；完整工作台、六个原生工具、PTY 保持、关闭重开、紧凑窗口与 80%/120%/140% 缩放真实检查通过；真实模型经桌面工具读随机标记并写入结果文件通过（隔离测试替代原生确认点击，非免确认权限）。
- 视觉审查发现旧双侧面板在窄窗口下遮挡发送区，已补充实际横向按钮边界检查和窄窗口单侧优先规则。最终包安装、原有语义数据、七项已安装检查和 4/4 匿名资产摘要通过；截图组件偶发失败的原证据保留，同一二进制在新配置重试通过。

## V1.0.3（已覆盖、已发布并匿名校验）

- 四种审查范围、真实行号批注的新增/编辑/删除、校验后回填输入框已经实现。
- 源码真实 Harness + Git + 主进程 IPC 测试通过；保留已有草稿、不自动发送、暂存/分支只读、缺失回合基线与紧凑面板均已覆盖。
- 审查批注仍为内存暂存；暂存/分支/回合范围不开放接受和恢复。V1.0.4 保留权限隔离的一体化工作台继续按计划推进。
- 最终包六项安装后验收通过，最终 app.asar 摘要 A974AEDCB90BEED4FD5540C52EAF4FFA4A3965FBAD8DA118DE171989085B47C2，安装与解包一致；4/4 公开资产匿名回下载一致。
- 最终源码 360/360 通过，新增未解决合并冲突的明确状态与接受/恢复禁用。首个包已覆盖，但补充冲突保护后的重建包才可发布；首个包摘要 D568FF278DCC69FAEEAC5300C8B277B3EE4A61D088B77858315B08DE08D8703C 不能冒充最终包。
- 从加密配置和 Chromium 加密状态副本验证解密成功；Key 未出现在密文文件、已安装 app.asar 和 Harness 日志中，原配置未修改。覆盖前后语义快照一致 E87346B8785527E43B339846DF16F3AF6105027F532858E3BC7F96A7A6E31EFD。

## V1.0.2（2026-08-31，已覆盖并发布）

- 连续推进至 V1.1；Stable V0.5.4 不变，远程 Codex 实验不在本计划。
- 已实现受控文档入口、富文本引用、原文件和会话切换保护、用户附件 Git 拒绝保护。
- 已实现 Windows DPAPI 凭据托管和迁移，软件 Key 优先级保持最高；旧明文组件停用、密文回读和组件激活检查均为迁移前提。
- 真实 Harness 富文本 + 本机 File + 拖放事件 + 上游发送负载测试通过（不等同于手工资源管理器拖放）。独立真实模型读取 Excel 合计、Word 随机标记通过；测试未修改原安装版 Key。
- 完整源码回归 350/350，零失败、零跳过；生产依赖审计未发现已知漏洞。
- 解包版及已安装版的文档入口、IPC 隔离、Harness 启动、Office 中心、真实两命令终端均通过；解包版真实模型 Excel/Word 读取通过。已安装版本登记为 1.0.2，安装与解包 app.asar 同为 F76D99E51360852BA788689C20266DA265C9AAFD00ED273FBC6329C18E1C4297。
- 覆盖前后 31 个语义文件完全一致（4721A70B8C9A5B0DE02AFB49643CFD8441F480B9E2CF17D7782717BF9811842C）；之后正常启动已完成原软件 Key 的加密迁移，不能把迁移后的全部目录宣称为逐字节不变。
- 打包治理 packageReady=true；安装器仍未签名，自动安装禁用；V1.0.1 回滚包保存在 backups/pre-v1.0.2-20260831。
- 验证中发现：共享测试 Harness 目录与不同 Chromium 用户目录搭配时无法解密，已改用成套独立测试目录；一次上游首次会话目录创建/扫描竞争报 ENOENT，同一隔离配置重试通过。后者纳入 V1.0.5 首次启动恢复门禁。
- PR #62、PR CI 33325812783、main CI 33325999737 全部通过；已发布 Pre-release v1.0.2，目标 cfc5d388f59898636383147281eeaeffd9f892cd。四个附件匿名回下载的大小和 SHA-256 全部一致，证据 artifacts/public-v1.0.2/verification.json。V0.5.4 Stable 未变化。V1.0.3 审查范围不属于本次包。

## 上一发布基线

> 日期：2026-08-30
> 当前构建：V1.0.1 产品 Latest（已覆盖安装并发布 GitHub Pre-release）
> 状态：V1.0.1 已完成源码、打包、回滚保护、覆盖安装、已安装应用验收、PR/CI、GitHub Pre-release 和匿名公开回下载；V0.5.4 继续保持 Stable 和 GitHub 正式 `Latest release`

## V1.0.1 当前进展（已安装并发布）

1. Git 变更审查核心新增显式状态分层：`Git 不可用`、`当前目录不是 Git 仓库`、`Git 状态读取失败`、`仓库干净`和`存在变更`不再共用空列表语义。
2. 仓库发现成功但基线或状态命令失败时，审查器失败关闭接受/拒绝动作，同时保留仓库身份，不再误报为“非 Git 仓库”或“没有变更”。
3. 工作台变更审查面板为每类状态提供独立标题和恢复说明；非 Git 与读取失败状态明确说明对话、Office、Wiki、文件查看和终端仍可使用。
4. “仅未跟踪文件”作为真实变更显示，不会归入仓库干净；刷新失败提供可见提示，不再静默吞掉异常。
5. Office 交付中心页头、集成链和帮助/关于统一读取当前应用版本；最终解包版与已安装版均明确显示 `V1.0.1`，不再残留固定 V0.6 文案。
6. 命令面板执行失败后保留可访问的错误状态、受限详情、重试和返回命令列表；不再对未知失败笼统声称“没有执行任何修改”，失败对话框把焦点限制在可见控件内，菜单快捷键复用同一套可见失败对话框。
7. Harness 尚未连接时，启动状态页、模型诊断和菜单仍可进入网络与代理设置；IPC 继续限制为受信主窗口和直连、Windows 系统、无凭据 HTTP(S) 代理三种模式。
8. 新增真实命令反馈验收：在 1220×800 和 2560×1344 最大化窗口中实际触发失败、验证重试焦点与正反向焦点循环，并从离线状态页打开已完成加载的“网络与代理”弹窗；截图等待界面完成绘制，避免把上一帧误当作验收证据。
9. 版本已提升为 1.0.1，并生成最终 Setup、blockmap 和 Portable；静默覆盖安装退出 0，Windows 登记为 `DSH Desktop 1.0.1`。
10. 实现经 PR [#60](https://github.com/hejiahang0001-oss/dsh-desktop/pull/60) 合并为 `b76e850330b9f3e037277a7f6f9d645387fd8841`；PR CI `33310280781` 与 main CI `33310326991` 全部通过。V1.0.1 已发布为 [GitHub Pre-release](https://github.com/hejiahang0001-oss/dsh-desktop/releases/tag/v1.0.1)，四个公开附件匿名回下载后大小和 SHA-256 为 4/4 一致，校验清单为 3/3 通过。

### V1.0.1 当前门禁

| 门禁 | 当前结果 |
|---|---|
| Git 状态与新界面专项 | 29/29 通过；覆盖 Git 缺失、非仓库、状态读取失败、干净/仅未跟踪文件、动态版本、真实命令失败/重试/返回/焦点循环和离线网络入口 |
| 完整源码回归 | 325/325 通过；0 失败、0 跳过 |
| 最终解包/安装回归 | 解包版和已安装版均通过 Desktop、IPC、Office、命令反馈/网络入口、Harness HTTP 200 和固定终端六组；1220×800 与 2560×1344 截图逐张检查，Office 两处版本均为 `V1.0.1`，终端工作目录与凭据隔离通过 |
| 生产依赖审计 | `pnpm audit --prod --audit-level=high`：未发现已知漏洞 |
| 打包治理 | `packageReady: true`；Harness 运行时 24,288 个文件、243,912,681 字节、250 个包、零重解析点；相对 V1.0.0 差分复用 99.5172% |
| 安装与数据 | 静默覆盖退出 0；安装/解包 `app.asar` 同为 `2172A13C10954B2DA5D712370AA2B469845170D2CE46063058279BC6CC96838D`；31 个语义文件在重做前、最终安装后和已安装回归后快照同为 `122F4C7E55FBE1883BB8E04C66FA3C3C2DDAF214D35107A5989BE97CBF055395` |
| 本地产物 | Setup 188,708,650 字节，SHA-256 `4D35BABFC97413D9184B20CA4FDC4C4C78092F17A0407DD2D36BCB5F737F75CA`；blockmap 194,760 字节，SHA-256 `C1588B977DC64376E533A0B6DD3C9655E2CF78A44538CFE45CB99063592F71B6`；Portable 188,017,150 字节，SHA-256 `AEFEE2C66764BA551CBC845528662771696C62FAC0F60099A225FDB407F90941` |
| GitHub 发布 | PR #60、PR CI `33310280781`、合并提交 `b76e850330b9f3e037277a7f6f9d645387fd8841`、main CI `33310326991` 均通过；V1.0.1 Pre-release 四个远端附件均为 `uploaded`，匿名公开回下载 4/4 摘要一致、清单 3/3 通过 |
| 回滚与发布边界 | `backups/pre-v1.0.1-20260830-170338` 保留 V1.0.0 回滚资产；`backups/pre-v1.0.1-redo-20260830-191216` 保留被替换的首个 V1.0.1 候选、治理报告和无凭据语义快照；安装器仍未签名，自动更新仍被阻止，V0.5.4 Stable 未晋升、未替换 |
...
1. 新增原生 Windows 托盘：显示固定 Agent 状态，提供打开主窗口、定位待确认、停止当前生成、手动检查产品 Latest 和显式退出；不展示对话、命令或工作区路径。
2. Agent 从运行转为等待、完成、存在失败、停止或连接中断时生成固定文案通知；同一轮等待只通知一次，历史失败计数不会被误算成本轮失败，点击通知只回到固定本机操作。
3. Agent 仍在运行或等待确认时关闭主窗口会隐藏到托盘继续监督；显式退出仍按现有顺序等待备份/Git 操作并停止终端、预览和 Harness。
