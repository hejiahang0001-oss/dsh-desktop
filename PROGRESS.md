...
> 日期：2026-08-25
> 当前构建：V0.5.8 Latest 候选（源码门禁进行中）
> 状态：V0.5.8 已完成上下文来源只读视图和定向测试，正在执行全量、打包、覆盖安装与真实界面门禁；V0.5.4 继续保持 Stable
...
| 全量自动化 | 待发布文档同步后重新执行 |
| 打包/覆盖/数据 | 待候选安装包完成后记录 |
| 真实界面 | 待最终安装版完成后记录 |
| 发布通道 | V0.5.4 保持 Stable；V0.5.8 通过全部门禁后仅发布 Pre-release |
