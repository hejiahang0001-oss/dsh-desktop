...
## V1.1.14 alpha.2 适配（2026-09-18，已接受 Office 引擎体积；未升级安装）

- 每日任务发现 `dsh-v0.1.6-alpha.2` / `ddefc45fbc7f8e46dd73185e68295696d1297887`，发布于 2026-09-17 13:30:16 UTC；GitHub 标签、干净源码检出和 npm 版本相符。比较与取舍见 [alpha.2 预检](docs/HARNESS_UPSTREAM_v0.1.6-alpha.2.md)。
- 准备分支 `codex/v1.1.14-harness-alpha2`。完成公开 Session binding 的导航兼容准备，保留 alpha.1；新旧接口专项 16/16 通过，完整源码结果见 [验证记录](docs/VALIDATION.md)。不以契约模拟测试代替新内核交互验收。
- 官方 Office 预览拟直接采用，结构安全/备份/收据继续保留；官方改动卡片是临时只读记录，不能取代 Git Review/恢复；插件热卸载和全 Profile 变更需要额外验证，暂不删除受控安装。
- 新增 Windows Office 引擎 npm 解压体积 340,863,092 字节（约 325.1 MiB），最终安装包增量未测。维护者已明确“接受”，继续固定安全锁、Windows x64 引擎与运行库构建；这不豁免 Portable 或发布许可验收。
- 源码候选已切换 V1.1.14 / alpha.2；[固定 Windows 构建 35300839543](https://github.com/hejiahang0001-oss/dsh-desktop/actions/runs/35300839543) 通过，302 个发布包和原生引擎 2,048 项摘要一致，实际运行库 596 个包名审计无已知发现。首次审计发现 Office 新引入的 fflate ZIP64 无限循环，精确升级到 0.8.3 并复现旧版失败/新版通过，失败证据保留。
- 已完成运行库下载摘要和本地独立审计；最终完整源码 **621/621** 通过，失败/跳过均为 0。许可证清单覆盖 599 个物理包身份及新 PDF 懒加载文件中的十份嵌入许可。首次集成测试的两项旧断言失败与修正证据保留，不删除或放宽来源校验。
- 真实源码态启动发现 alpha.2 不再查找旧的共享 `profiles/node_modules`，已将三个固定桌面桥接插件改为官方支持的 `DSH_HOME/node_modules` 后备位置，保护链接路径并保留旧副本；未修改上游应用源码或用户 Profile 配置。
- 隔离原生转换、取消、防覆盖与源文件保留通过；真实侧栏也通过 DOCX 两页、XLSX 一页及 PPTX 三页中文预览，逐页内容/截图与源文件摘要已检查，既有五类预览仍通过。深层 pnpm 的 Windows 文件名过长已定位，实际源码平铺路径通过；任意自定义安装深度和其他电脑未验证。Aptos 字体替代仍可见，不宣称排版与字体完全一致。
- 源码测试和预览完成不等于整包交付：alpha.2 的插件热卸载/全 Profile、真实模型插话、Setup/Portable、覆盖安装及数据保留尚待验收；Office kit 声明的源码仓库返回 404，Node API 对应源码可获取性仍待核实，暂不公开新包。维护者已允许询问官方；因官方关闭 Issues 并指定 Discussions 接收反馈，已在官方 Q&A 提交 [#7026](https://github.com/deepseek-ai/deepseek-harness/discussions/7026)，正文与目标回读一致，等待答复。
- 本机仍为 V1.1.13 / alpha.1；GitHub v1.1.13 仍是四资产私有草稿，原 Portable 失败证据和发布范围选择保留；Stable V1.1.12 不动。Obsidian 按本次实质进展同步，不能写成 alpha.2 已交付。
- 路线调整为：V1.1.14 alpha.2 适配 → V1.1.15 恢复/便携性能 → V1.1.16 最终老化 → V1.2.0 双机 Beta。若保留便携交付，必需修复不能以排期顺延豁免发布门禁。

## V1.1.13 官方能力接入（2026-09-16，已安装；公开发布暂缓）

- 用户同意按官方能力对比建议迭代。固定 `dsh-v0.1.6-alpha.1` / `0a15e36e7f82b6ed45af6fa9759f29b40dcd965d`；分支 `codex/v1.1.13-harness-alpha1`。Stable V1.1.12 不动。
- 602/602 基线、冻结安装、生产依赖审计通过。新内核来源清单为 285 DSH + 9 Cordis，8 项精确安全修复已在新锁文件重建；[固定 Windows 运行库构建](https://github.com/hejiahang0001-oss/dsh-desktop/actions/runs/34992044265) 成功，下载摘要一致，本地 557 包名物理审计无已知发现。
- 官方终端公共导航、精确主会话校验、兼容终端标识与关闭额外原始日志上报已实施；19 项相关测试通过。官方 follow 会抢占输入控制，因此保留兼容终端用于逐次授权/脱敏读取，不接私有状态、不强删能力。
- 完整源码测试 608/608、审查后专项 26/26 通过。修复自动检查点将终端 textarea 误当聊天框、截走回车的问题；现在严格绑定官方聊天卡，不跨卡查找发送按钮。审查同时补齐产品/包门禁版本一致性断言。
- 官方多终端真实键盘执行、工作区绑定、窄窗口、独立终端与明确关闭通过；官方归档取消与重载保持通过。V3 冷读 22 个事件、5 页历史及并发读写边界通过。加密软件 Key 的真实 Excel/Word 附件读取通过，源 Key 和文档未改变、日志无明文 Key。
- 最终整包与本机安装版各 10 组验收通过，包含文件拖入/五类预览、三页中文 PDF、真实 Queue/向上插话/Ctrl+Enter/Stop/present、终端/归档、Office/Wiki 与权限隔离。包载荷绑定通过，GitHub CI 三项通过。备份后静默覆盖到 V1.1.13，41 份语义文件和三个加密相关文件安装前后不变；安装版真实 Excel/Word 读取另行通过。
- 安装内容 24,989 个文件与最终验收包逐字节一致，所有测试后 41 份语义文件和三个加密相关文件不变。GitHub PR #83 已合并，主分支 CI 通过；草稿 389338033 的四项资产上传完整并核对摘要，临时传输资产已从草稿删除，本地保留。
- 便携版未验收通过：两次 Harness 基础烟测在进程清理时报 code 128、未写最终回执；独立真实终端检查也在命令回显环节超时。启动至测试结束约五分钟，不能以启动器退出码 0 代替验收。已询问维护者选择“本版只公开已验收安装版”或“修好便携版后一起公开”；当前保留私有草稿，不改 Stable。
- 附件菜单和 MCP resources 随官方内核提供；第三方 MCP 服务不宣称已逐个验证。公开下载与 Wiki 的公开状态同步暂未执行，不把已安装/已上传草稿当成已公开。
- 按本轮发现调整路线：Office 恢复交互/Portable 性能 → V1.1.14，最终功能集老化 → V1.1.15。下一步先验证新内核和安装包，不继续叠加功能。
...
- 目标：采用官方文档预览、`present` 文件交付、Flash 模型接入及可靠性修复；保留 Office 校验/备份回执、Wiki、Git Review、软件 Key 和代理边界。
- 独立分支 `codex/v1.1.12-harness-rc.2`；旧运行库、安装包、用户数据保留。V1.1.11 冻结安装、生产依赖审计和 595/595 测试通过（本轮新跑 239.7 秒）。
- 固定运行库构建/下载摘要通过；602/602 完整测试、生产依赖/物理运行库/新增 PDF 浏览器依赖审计通过。537 个物理许可身份与十份 PDF.js 嵌入许可核对通过。
- 五类官方预览、三页中文 PDF 和键盘搜索通过。修复桌面入口引用不存在的工作区变量；会话/工作区/敏感路径/链接校验保留。删除重复普通预览 UI，保留 Office 校验和恢复回执。
- 真实 Queue、上箭头、Ctrl+Enter、Stop 与 present 交付/预览通过；侧栏脚本补齐关闭已有标签和刷新目录缓存后，全屏/窄窗控件命中通过。没有修改上游源码。迁移、独立备份回退、零 Agent 冷读及打包版六项无 Key 烟测通过。
- 最终 Setup/Portable 载荷门禁、包/源码指纹、原生终端、双实例、安全退出和 Portable 实际启动通过。指纹 `1bfe1e993dd690061fa44372c9ca680761aa20aac84dd62222364b19ef3bae0f`；两个安装包和校验文件在 `dist`。
- PR #82、发布代码 main CI、传输工作流通过；原草稿 `386457813` 的四项资产大小和 SHA-256 与本机一致，临时差分资产已移除且本地保留。已公开 [V1.1.12 Pre-release](https://github.com/hejiahang0001-oss/dsh-desktop/releases/tag/v1.1.12)，标签指向 `8ebccb74554d139354e12c8ffe4b14febd3327c6`，没有晋升 Stable。
- 用户确认退出后核实无旧版进程，备份 38 份语义文件，静默覆盖 1.1.11.0 → 1.1.12.0，安装器退出码 0。安装态六项基础、五类预览、三页中文 PDF、键盘搜索、真实 Queue/向上插话/Ctrl+Enter/Stop/present 与 Excel/Word 读取通过；安装内容指纹与最终包一致。测试使用独立资料，38 份语义文件及三个加密相关文件安装前后、测试后均不变。
- [独立匿名下载 34679978281](https://github.com/hejiahang0001-oss/dsh-desktop/actions/runs/34679978281) 完整下载四项资产并核对大小/摘要；本机下载的校验清单 3/3 与原始构建和独立下载结果一致。当前只收口 V1.1.12，不自动开始下一版。
- Office 恢复交互/Portable 性能顺延至 V1.1.13，功能冻结后的至少 24 小时老化顺延至 V1.1.14，不混入本次兼容切片。

## V1.1.11 Harness V3 / 官方文件面板（2026-09-09，已安装并公开）

- 用户确认按官方对比结论推进一个 Latest 版本。固定 0.1.5-alpha.1，官方桌面壳、Office 恢复交互和 Portable 性能不混入本轮；Stable V1.1.0 不动。
- V1.1.10 基线冻结安装、生产依赖审计、585/585 测试和 Windows 解包构建通过；新内核通过独立分支构建，不覆盖旧运行库。
- Inbox 漏检回归先失败后通过；改用 nextTurn / nextStep。删除桌面重复目录浏览，保留搜索、媒体与审查入口；13 项界面/布局测试通过。
- 最终 595/595 测试通过；实际运行库审计、V2→V3 明文/压缩迁移、锁与备份回退、零 Agent 冷读、真实插话/停止、双文件标签和真实 Excel/Word 读取通过。修复官方自动全屏被桌面审查面板遮挡的问题，仅调整宿主边界。
- 最终 Setup/Portable 载荷和发布门禁通过；本机从 1.1.10 覆盖至 1.1.11，安装后六项烟测、真实模型和完整包指纹通过，36 份语义数据及三个加密相关文件不变。PR 81 和主分支 CI 通过；四资产已完整公开，独立运行器匿名完整下载 4/4、校验清单 3/3 通过，Stable V1.1.0 不动。
- 上传链路较慢，采用 15.9 MB 字节差分包还原本机已验收的两个原始安装包，完整 SHA-256 不变；不是重新构建。临时传输资产已从发布页移除。下一版为 V1.1.12 Office 恢复/Portable 性能，之后再进行最终功能集至少 24 小时老化。

## V1.1.10 官方文件流程与 Harness 适配（2026-09-08，本机已安装，Latest 已公开）
...
- 已适配 Session v2 冷读/锁/迁移、personaSuffix、软件代理优先级和官方附件交互。最终 585/585 源码测试、真实冷读、合成会话迁移、三格式交付、真实模型官方附件读取和打包生命周期均通过；已覆盖本机到 V1.1.10，安装态附件/真实读取通过，36 个语义文件及 3 个加密相关文件保持不变。
- 原 Office 恢复与 Portable 性能切片顺延，不混入本次内核升级。Stable 仍为 V1.1.0。
- 用户确认按建议完成一个 Latest 版本：移除默认“导入工作区”按钮及空栏，保留旧引用、草稿和按需格式 Skills；不重做整个任务中心。
- 上游 monorepo 31 项告警按实际打包范围重新核对：七个受影响依赖采用精确同主版本补丁，undici 8.10.0 不在该批 7.x 告警范围，其他三个包未打入运行库。固定安全锁文件与构建来源，物理运行库审计不通过则不发布；见[安全审查记录](docs/HARNESS_ALPHA2_SECURITY_REVIEW.md)。
- 固定安全运行库与桌面生产依赖审计均无已知漏洞；这不等于整个官方 monorepo 没有告警。PR #79 与 main CI 均通过，四项资产全部上传并核对大小/摘要后已公开 V1.1.10 Pre-release；匿名完整下载 4/4、下载的校验清单 3/3 通过，Stable 仍为 V1.1.0。
- 七页版本/能力/兼容/路线 Wiki 已合并更新，保留历史版本和其他项目内容。下一版记录明确会话导航及打包前 mjs 换行核验，继续处理恢复交互和 Portable 性能；本轮在 V1.1.10 收口，不自动开始下一版。

## V1.1.9 Office 安全检查与真实交付收据（2026-09-07）

- 本轮只交付一个小版本：统一 Word/Excel/PPT 的 ZIP、XML、关系、活动内容和敏感路径边界，补齐 PPT 内嵌工作簿公式检查。
- 创建及受支持修改返回实际文件的相对路径、大小、SHA-256、结构安全结果和覆盖备份摘要；检查不通过不发布新产物，不将结构通过表述成视觉验收。恢复卡片和持久化恢复状态留待下一版。
- PDF 原生验收改为三页中文、嵌入字体与逐页截图，不新增任意 PDF 编辑能力。
- XML 解析器固定为 `@xmldom/xmldom@0.9.12`，同步锁文件、独立 Skills 运行依赖和 531 包许可证清单。
- 已检查官方 `dsh-v0.1.3-alpha.1`：其上传/代理等能力与桌面存在交集，但引入异步会话锁、Session v2 及已知历史加载回退；本次不混入内核迁移，保持 rc.1，适配前不删除现有入口。
- 最终测试、安装和发布证据集中维护于 [验证记录](docs/VALIDATION.md)；完成状态以实际回读为准，Stable 不变。
- 已通过完整源码 545/545、最终变更门禁 32/32、生产依赖审计、三格式独立 CLI、真实 DeepSeek 读取、三页中文 PDF、包内容一致性和生命周期检查。已从本机 V1.1.5 覆盖到 V1.1.9，安装态专项通过，36 个语义文件和 2 个加密凭据文件均保持不变。
- GitHub PR #77 和 main CI 均通过，合并提交 `11446ac39a74dcc1dbb914dd9a4d8bf8f12f3bc8`；V1.1.9 已公开为 Pre-release，四项资产远端状态、大小和摘要一致，匿名完整下载 4/4、下载的校验清单 3/3 通过。没有开启定时监控或晋升 Stable。本轮到此停止，不自动继续 V1.1.10。

### V1.1.8 后续发布更正

V1.1.8 已从本地候选推进到 GitHub Pre-release，目标提交 `4602aca7fcf76cf6e169dd73bd50201ed2161118`。四项资产的远端状态、大小、摘要及匿名完整下载均已验证，证据为 `artifacts/v1.1.8-release-candidate/github-publication-verification.json`。下方“未发布”为当时收口记录，不代表当前发布状态。

## V1.1.8 Wiki 知识库产品化（2026-09-05，本地候选收口，未安装、未发布）

- Wiki 中心统一首次引导、健康/来源状态、本地查询、会话结论、项目增量、DSH 历史和六页版本知识入口；继续使用固定 Harness `0.1.2-rc.1`。
- Wiki 写入共享 Vault 级锁，恢复使用受保护的追加日志和精确对象校验。Windows 路径别名、rename/link 完成歧义及并发人工修改均有回归覆盖；项目总览和历史页面缺少已提交 SHA-256 时拒绝覆盖既有页面。
- 2026-09-05 完整源码回归 **536/536**，失败 0、跳过 0，耗时 214.2664 秒；其中包含此前 71 项 Wiki 聚焦回归。生产依赖审计刷新成功，已知漏洞为 0；此前 npm registry 超时已不再阻塞本次收口。
- 构建器固定为 electron-builder 26.11.1，包内容检查固定使用 7zip-bin 5.2.0；保留 Windows 版本信息编辑，候选仍未签名。`afterPack` 仅移除大小与哈希均精确匹配的 Electron 默认应用。
- 最终候选包指纹为 `37254f32d9c8d14ed0e957bccda5543186162f2f7838bae030f32e328773eecd`；`app.asar` 130/130、资源 24,358/24,358 匹配。Setup、Portable 和展开目录的 24,433 个文件、734,069,182 字节逐项一致。
- 打包态 Wiki CLI、GUI 和普通展开版双实例生命周期检查通过。安全退出验证修复了 stdout 尾帧读取及 30 秒等待不足的问题：一次性授权固定为 180 秒，最终应用/guardian 均退出 0，受管进程及端口残留为 0，双份生命周期记录为 clean。
- Portable 真实基础启动/退出成功，耗时 235,970 毫秒（约 3 分 56 秒）。临时 Portable 双实例入口因外壳/内层 PID 模型不适用已撤回；本版不声称 Portable 双实例生命周期通过，冷启动优化列为后续事项。
- 发布治理 `packageReady=true`；相对 V1.1.7 可复用字节 98.0643%，预计差分下载 3,652,245 字节。未签名候选保持 `automaticUpdateReady=false`；精确资产大小、SHA-256 和验证路径统一记录在 [验证记录](docs/VALIDATION.md)。
- 交付目录：`artifacts/v1.1.8-release-candidate`，包含 Setup、Portable、blockmap、SHA256SUMS 和本次验证证据。本轮没有覆盖安装、GitHub 推送或发布，也未修改 Stable。
- 按用户 2026-09-05 指令，本轮在 V1.1.8 收口；V1.1.9–V1.2.0 仅保留路线，不自动继续。
...
- 精确比较 alpha.5→rc.1 的 2 个提交、252 个文件：全部为 `package.json` 的版本号变化，没有 Agent、Remote、Session、Web UI、配置或锁文件逻辑变化。官方 rc.1 发布说明是从 0.1.1-rc.2 起的累计摘要，不据此再次删除桌面功能。
- 官方源码构建、242 个 DSH 家族发布包、9 个 Cordis 支撑包及 39 个编译伴随 Loader 校验通过；源部署为 251 个发布包、23,866 文件、244,474,542 字节、零重解析点，CLI 返回 `0.1.2-rc.1`，未发现 alpha.5 混装。
- 增加完整适配门禁：预发布插件版本不丢失 alpha/rc 身份；打包来源同时匹配官方 repository/tag/full commit；固定 242+9 发布包及 3 个辅助包身份；实际运行时代码、`app.asar` 版本、法律文件与构建声明逐项做内容绑定。
- 打包版只允许从应用 resources 解析 Node、DSH 和桌面补丁，忽略并清除三个开发覆盖环境变量；内置资源缺失时失败关闭，不再退回源码目录或外部伪造文件。固定 DSH 入口为顶层 `@deepseek-ai/dsh/lib/bin.js`，不选择 `.pnpm` 阴影副本。
- 首个解包候选被内容门禁正确拦截：构建阶段 11 个文件被误计为分发负载。修正后只绑定实际分发的 `node_modules`，最终 23,854 个负载文件、243,748,493 字节与 SHA-256 `77e4f3ee552fbbf830bb334e58b03d5c53c1b00879583d0dc1b5a02d42081d05` 在来源和包内完全一致。
- 首轮 Harness 解包冒烟又发现 `bufferutil`、`utf-8-validate` 和 `@cfworker/json-schema` 三个 optional peer 被误判为缺失；未降低健康标准，而是只排除明确 `optional: true` 的 peer，保留普通依赖和必需 peer 缺失失败关闭。最终闭包 491/491 健康，152 个插件项、123 活跃、0 失败，三个必需模块均活跃，Workspace/Session 与独立 Side Chat 正常。
- 五个隔离复制的已完成 JSONL-Zstd 会话在 alpha.5 与 rc.1 下事件数和历史哈希逐项一致；源文件 5/5 未改变，不复制凭据、不调用模型。许可清单仍为 530 项、8 类。
- 最终源码门禁 411/411、零失败/跳过；219 个实际存在的版本控制及本版新增 JavaScript 文件通过语法检查，生产依赖审计和空白检查通过。最终解包版基础交互 8/8、独立终端、跨工作区真实模型 3/3 均通过；真实执行官方队列向上按钮、`Ctrl+Enter` 插话和 Stop，Office 原文件不变，草稿保持会话隔离。
- Setup、Portable 与 blockmap 已生成，校验清单 3/3 匹配。Setup 为 188,643,588 字节、SHA-256 `c084cc1b255cd7914b737de027784610779773759bf9b9474fbb99b7181c5469`；Portable 为 187,952,074 字节、SHA-256 `b5b0037f407ef7c13c8012f32022305433f8fe0906c540dd0459c3fc7fc99515`。包治理 `packageReady=true`，相对 V1.1.5 blockmap 复用率 96.5027%；安装器仍未签名，可信自动安装保持关闭。
- V1.1.6 现为已验收的本地可安装候选。尚未关闭或覆盖当前 V1.1.5，也未执行安装态数据保留验收；覆盖前仍需用户即时确认。尚未推送或发布 GitHub，V1.1.0 Stable 与公开 V1.1.1 Pre-release 均不变。

## V1.1.5 工作台 UI 精修（2026-09-03，已覆盖安装，本地验收完成，未发布）

- 保持 Harness alpha.5 官方会话、排队、插话、停止和输入框结构不变，只修改桌面拥有的 Dock、Office 文件入口、Review 和布局样式。
- 底部工作台已区分主工具、辅助面板和窗口操作，主标签在窄宽度可横向滚动，已打开工具有弱状态提示；文件入口与输入框宽度、表面和焦点层级统一。
- 空仓库、受保护或没有可执行变更时不再显示接受/拒绝按钮；有真实可执行变更时继续走原有确认与 Git 安全边界。
- 已补高对比度、系统强制色、减少动画、焦点可见性和文件提示的辅助技术关联；清理 V1.1.4 删除自定义插话后遗留的无效样式。
- 12/12 定向 UI 测试通过；完整源码门禁最终 406/406、零失败/跳过，生产依赖无已知漏洞。第一次完整门禁有一个环境插件测试文件在并发下进程级失败且没有子项错误；该文件单测通过，随后未经代码修正的完整复跑 406/406 通过，失败事实保留。
- 源码真实 Harness Dock、文档拖入与 Review smoke 均通过；已人工检查紧凑、Office、四文件引用和 Review 合成截图。最终解包版与已安装程序的基础矩阵均 8/8 通过，覆盖文档原生拖入、草稿连续性、Git Review、隔离 IPC、Office、PDF 和真实 PTY。
- 发布布局检查 `packageReady=true`，固定 Harness 为 `0.1.2-alpha.5` / `db6bdc3576c2d4e7c965e8e3ed0c2a731eed87f5`，零重解析点；相对 V1.1.4 blockmap 复用率 99.4892%。Setup 为 188,638,503 字节、SHA-256 `28c18ffeb762b87c1c054bd4fc5e5fdceed786b924cc0f7f432ac623b7f75609`；Portable 为 187,947,003 字节、SHA-256 `407d497ed9c0352dbe1d3a9714b112cbbc1228fbdd118aae0278b2f865bc9908`。安装包仍未签名，自动安装继续关闭。
- 本机已由 V1.1.4 覆盖到 V1.1.5，安装器退出码 0，Windows 登记和五项核心资源哈希一致。覆盖前 34 个会话/设置文件已形成可验证备份且不含凭据文件；安装前后语义快照、加密 Key 和 Local State 哈希均不变。
- 当前仍是本地开发分支，GitHub 未发布；V1.1.0 Stable、公开 V1.1.1 Pre-release 和上一版 V1.0.5 不变。

## V1.1.4 官方能力去重（2026-09-03，已覆盖安装，本地验收完成，未发布）

- 对照官方 alpha.3、alpha.4、alpha.5 发布说明与固定源码完成能力归属审计；完整矩阵见 `docs/HARNESS_UPSTREAM_OVERLAP_v1.1.4.md`。
- 已确认官方原生支持排队、排队消息向上按钮、`Ctrl+Enter` 插话和停止；桌面自定义层属于完全重复，并且会在侧栏工作区切换后拦截官方按钮、产生错误红字。
- 已删除自定义插话控制器、渲染拦截脚本、重复操作条、三项桌面 IPC 与私有 `resume-queue`；新增所有权回归测试，防止后续重新引入。
- Office 文件拖入、文档引用连续性、隔离后台任务、Tasks/Subagents 管理面、工作区/工作树交接、软件 Key 和代理设置不属于完全重复，继续保留。
- 真实模型验收直接操作官方 QueueDock 向上按钮、`Ctrl+Enter` 与 Stop，源码、unpacked、跨工作区和安装态均 7/7 通过；随机标记文件证明两条插话实际执行，不再调用被删除的桌面接口。
- 去重时发现 Tasks/Subagents 仍需要只读工作流状态；先由回归测试复现 `getCurrentWorkflow is not defined`，随后只恢复只读状态查询，不恢复任何自定义排队或插话。首次官方控件验收脚本只在输入框内找按钮，真实排队其实已完成；修正为定位官方 QueueDock 后完整通过，失败证据保留。
- 最终源码 402/402、零失败/跳过，生产依赖审计无已知漏洞。测试数量低于 V1.1.3，是因为删除了两组重复控制器专项，改为官方所有权和官方控件验收，不是跳过测试。
- 源码、unpacked 和已安装程序的基础矩阵均 8/8；真实模型矩阵均 4/4；跨工作区文档/连续性/官方插话均 3/3；打包与安装终端隔离均通过。发布布局 `packageReady=true`，安装包内不存在已删除的重复脚本。
- Setup 为 188,637,856 字节、SHA-256 `cbb4340f1af7a6a490caf62b74527d878ee538e10b9b6e4bcf6b766dfe2d2840`；Portable 为 187,946,356 字节、SHA-256 `5825c78a54ade5b67c940ef0d79d123593684aa5c555840e120d61057ab92ca9`。相对 V1.1.3 的 blockmap 复用率为 99.473%，但安装包仍未签名，自动安装继续关闭。
- 本机已由 V1.1.3 覆盖到 V1.1.4，系统登记和五项核心资源哈希一致；覆盖前 34 个会话/设置文件已形成可验证备份且不含凭据文件，安装前后语义快照、加密 Key 和 Local State 哈希均不变。
- 当前仍是本地 Latest：V1.1.0 Stable 与公开 V1.1.1 Pre-release 未改动，GitHub 未发布。
