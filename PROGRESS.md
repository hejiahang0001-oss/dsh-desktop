...
> 日期：2026-08-25
> 当前构建：V0.5.16 产品 Latest 候选（本机门禁完成，等待 CI 与远端发布核验）
...
6. 本机发布门禁已完成；代码审核补充确认后仓库绑定、Git 执行覆盖隔离、总数上限、恢复点复用、原子所有权、仿冒只读、失败回收恢复和可信视觉截屏。CI 与远端三资产核验待完成。
...
| 发布状态 | 本机门禁完成；PR/CI、GitHub Pre-release 和远端三资产核验待完成；V0.5.4 仍为 Stable 与正式 Latest |
