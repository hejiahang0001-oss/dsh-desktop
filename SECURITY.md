...
- The software-managed credential is encrypted at rest through Windows DPAPI in `.credentials.dpapi.json`. DSH verifies the encrypted provider before disabling and removing an unchanged legacy `.credentials.yaml`; no plaintext rollback copy is created.
...
Engineering safeguards and the required review sequence are documented in [docs/DEVELOPMENT_PLAYBOOK.md](docs/DEVELOPMENT_PLAYBOOK.md).
