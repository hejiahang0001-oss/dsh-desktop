...
test('extension center maps the official inventory into four fixed capability surfaces', () => {
  const state = buildExtensionCenter({
    ...
    runtimeCapabilities: {
      skills: { status: 'ready', version: '0.1.1-rc.2' },
      mcp: { status: 'ready', version: '0.1.1-rc.2' },
      pluginInventory: { status: 'ready', version: '0.1.1-rc.2' }
    },
...
test('installed MCP capability remains ready without claiming a configured server', () => {
  const state = buildExtensionCenter({
    runtimeVersion: '0.1.1-rc.2',
    runtimeCapabilities: { mcp: { status: 'ready', version: '0.1.1-rc.2' } },
...
test('extension center fails closed when live Harness inventory is unavailable', () => {
  const state = buildExtensionCenter({ runtimeVersion: '0.1.1-rc.2', inventoryError: 'Harness 尚未就绪。' });
