...
test('V1 license inventory is fixed to the packaged Harness and desktop runtime', () => {
  ...
  assert.match(script, /harness-hoisted-0\.1\.3-alpha\.2/);
  assert.match(script, /Expected 545 packaged JavaScript packages/);
  assert.match(inventory, /@deepseek-ai\/dsh@0\.1\.3-alpha\.2/);
  assert.match(inventory, /JavaScript packages inventoried: \*\*545\*\*/);
