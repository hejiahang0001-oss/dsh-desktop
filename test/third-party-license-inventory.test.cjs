...
test('V1 license inventory is fixed to the packaged Harness and desktop runtime', () => {
  ...
  assert.match(script, /harness-hoisted-0\.1\.2-alpha\.1/);
  assert.match(script, /Expected 529 packaged JavaScript packages/);
  assert.match(inventory, /@deepseek-ai\/dsh@0\.1\.2-alpha\.1/);
  assert.match(inventory, /JavaScript packages inventoried: \*\*529\*\*/);
