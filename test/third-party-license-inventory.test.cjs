...
test('V1 license inventory is fixed to the packaged Harness and desktop runtime', () => {
  ...
  assert.match(script, /harness-hoisted-0\.1\.6-alpha\.1/);
  assert.match(script, /Expected 560 packaged JavaScript manifest identities/);
  assert.match(inventory, /@deepseek-ai\/dsh@0\.1\.6-alpha\.1/);
  assert.match(inventory, /Physical JavaScript manifest identities inventoried: \*\*560\*\*/);
